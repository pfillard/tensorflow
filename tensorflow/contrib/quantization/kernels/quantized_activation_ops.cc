...
template <typename T>
class QuantizedRelu1Op : public OpKernel {
public:
    explicit QuantizedRelu1Op(OpKernelConstruction* context)
        : OpKernel(context) {}
    ...
    void Compute(OpKernelContext* context) override {
        ...
        OP_REQUIRES_OK(context,
            context->allocate_output(0, input.shape(), &output));
        const T min_as_quantized = FloatToQuantized<T>(0.0f, min_input, max_input);
        const T max_as_quantized = FloatToQuantized<T>(1.0f, min_input, max_input);
        output->flat<T>().device(context->eigen_cpu_device()) =
            input.flat<T>()
            .cwiseMax(min_as_quantized)
            .cwiseMin(max_as_quantized)
...
REGISTER_KERNEL_BUILDER(Name("QuantizedRelu1")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<qint32>("Tinput")
                            .TypeConstraint<qint32>("out_type"),
                        QuantizedRelu1Op<qint32>);
REGISTER_KERNEL_BUILDER(Name("QuantizedRelu1")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<quint8>("Tinput")
                            .TypeConstraint<quint8>("out_type"),
                        QuantizedRelu1Op<quint8>);
}  // namespace tensorflow
